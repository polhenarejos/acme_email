...
def _safe_load_pem_x509_certificate(cert_pem):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", CryptographyDeprecationWarning)
        return x509.load_pem_x509_certificate(cert_pem)

def _safe_load_der_x509_certificate(cert_der):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", CryptographyDeprecationWarning)
    ...
    if system == 'windows':
        ...
        for cert_bytes, encoding, is_trusted in items:
            if encoding == "x509_asn":
                try:
                    yield _safe_load_der_x509_certificate(cert_bytes)
                except ValueError:
                    continue
        ...
        for cert_file_name in certs_file_names:
            ...
            with open(cert_file_path, 'rb') as f:
                cert_pem = f.read()
                try:
                    yield _safe_load_pem_x509_certificate(cert_pem)
                except ValueError:
                    continue
        ...
        raise NotImplementedError(f'missing implementation for this operating system="{system}"')
